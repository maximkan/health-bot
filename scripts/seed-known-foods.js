// Parses /tmp/ns_foods.csv → SQLite known_foods + mirrors to Notion
...
function parseLunch(lines, weekLabel) {
  ...
  for (const cols of lines) {
    ...
    if (col1==='Item' || !currentDay) continue;
    ...
    entries.push({ name:`${currentItem} — ${sizeName} (${col3}) [${weekLabel}]`, day:currentDay, serving:`1 plate ${sizeName}`, calories:Math.round(calories), carbs:Math.round(parseFloat(cols[5])||0), protein:Math.round(parseFloat(cols[6])||0), fat:Math.round(parseFloat(cols[7])||0), notes:`Lunch ${weekLabel} | ${currentProtein} | ${currentDay}`, source:'Network School' });
  }
  return entries;
}
...
function parseDinner(lines, weekLabel) {
  ...
  for (const cols of lines) {
    ...
    if (col1==='Item'||col0.startsWith('All nutrition')||!currentDay) continue;
    const cats=['Base','Veggie','Protein','Vegan','Vegetarian','Sauce'];
    if (col0&&cats.some(w=>col0.startsWith(w))) { currentCategory=col0.trim(); continue; }
    if (!col1||isNaN(calories)) continue;
    entries.push({ name:`${col1} [Dinner ${currentDay} ${weekLabel}]`, day:currentDay, serving:'100g', calories:Math.round(calories), carbs:Math.round(parseFloat(cols[3])||0), protein:Math.round(parseFloat(cols[4])||0), fat:Math.round(parseFloat(cols[5])||0), notes:`Dinner ${weekLabel} | ${currentCategory} | ${currentDay} | per 100g`, source:'Network School' });
  }
  return entries;
}
...
function parseCafe(lines) {
  const entries = [];
  const hdrs=['Food','Smoothies','Cold-Pressed Juices','Espresso-Based','Non-Coffee Beverages','Dessert'];
  let section='Food';
  for (const cols of lines) {
    const col0=cols[0]?.trim();
    if (!col0) continue;
    if (hdrs.includes(col0)) { section=col0; continue; }
    if (cols[1]?.trim()==='Calories (kcal)') continue;
    const calories=parseFloat(cols[1]);
    if (isNaN(calories)) continue;
    entries.push({ name:`${col0} [NS Cafe]`, day:null, serving:'1 serving', calories:Math.round(calories), carbs:Math.round(parseFloat(cols[2])||0), protein:Math.round(parseFloat(cols[3])||0), fat:Math.round(parseFloat(cols[4])||0), notes:`NS Cafe | ${section}`, source:'Network School' });
  }
  return entries;
}
...
async function mirrorToNotion(entry) {
  if (!NOTION_DB) return;
  const props = { 'Food Name':{title:rt(entry.name)}, 'Source':{select:{name:entry.source}}, 'Serving Size':{rich_text:rt(entry.serving)}, 'Calories':{number:entry.calories}, 'Protein (g)':{number:entry.protein}, 'Carbs (g)':{number:entry.carbs}, 'Fat (g)':{number:entry.fat}, 'Notes':{rich_text:rt(entry.notes)} };
...
async function main() {
  ...
  if (!fs.existsSync(csvPath)) { console.error('❌ /tmp/ns_foods.csv not found'); process.exit(1); }
  ...
  for (const cols of allLines) {
    ...
    if (h==='DINNERS EVEN')      { current='dinnerEven';continue; }
    if (h==='NS CAFE')           { current='cafe';       continue; }
    if (current) sections[current].push(cols);
  }

  const all = [
    ...parseLunch(sections.lunchEven,'Even Week'), ...parseLunch(sections.lunchOdd,'Odd Week'),
    ...parseDinner(sections.dinnerOdd,'Odd'),      ...parseDinner(sections.dinnerEven,'Even'),
    ...parseCafe(sections.cafe),
  ];
  console.log(`Parsed ${all.length} entries from CSV`);

  // Load existing Notion names to avoid duplicate Notion entries
  ...
  console.log(`✅ Notion: ${notionAdded} added`);
